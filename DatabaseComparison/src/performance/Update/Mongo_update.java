...
public class Mongo_update extends Abs_update {
	...
	//@param employee number, salary, from date and to date of the existing document and 
	//the new salary that we want to update.
	public void updateEmployee(int emp_no, int salary, int newSalary,Date fromDate, Date toDate) {
		...
		while(cursor.hasNext()) {
			...
			if (!cursor1.hasNext()){
				System.err.println("Error while update: collection does not have this record to update.");
		...
		System.out.println();
		System.out.println("(Update) Time to execute = " + runTime + " milliseconds");        
